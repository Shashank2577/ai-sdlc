#!/usr/bin/env bash
# Tests for the security veto: scripts/security-gate.sh and
# scripts/security-verdict.sh. Mirrors scripts/test-qa-enforcement.sh
# exactly, s/qa/security/.
#
...
#
#   bash scripts/test-security-enforcement.sh
set -uo pipefail
...
import json,sys
n=int(sys.argv[1])
print(json.dumps([{'event':'labeled','label':{'name':'security:rejected'},
...
# ---------------------------------------------------------------------------
echo "scripts/security-gate.sh"
# ---------------------------------------------------------------------------

setup; issue 42 "type:story"; pr_body "Closes #42"
PR_NUMBER=42 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "passes when the linked item has no verdict" 0 $? "$WORK/out" "Security gate passed"

setup; issue 42 "type:story" "security:rejected"; pr_body "Closes #42"
PR_NUMBER=42 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "fails when the linked item is rejected" 1 $? "$WORK/out" "#42 carries \`security:rejected\`"

setup; issue 42 "security:approved"; pr_body "Closes #42"
PR_NUMBER=42 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "passes when the linked item is approved" 0 $? "$WORK/out" "Security gate passed"

setup; pr_body "Relates to #42, see also #43"
PR_NUMBER=1 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "no closing keyword means nothing to honour" 0 $? "$WORK/out" "closes no work item"

setup; issue 42 "security:approved"; issue 43 "security:rejected"; pr_body "Fixes #42 and closes #43"
PR_NUMBER=1 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "any rejected item among several blocks" 1 $? "$WORK/out" "#43 carries"

setup; issue 42 "security:rejected"; pr_body "CLOSES #42"
PR_NUMBER=1 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "closing keyword match is case-insensitive" 1 $? "$WORK/out" "#42 carries"

setup; issue 42 "security:rejected"; pr_body "Resolved #42"
PR_NUMBER=1 bash scripts/security-gate.sh > "$WORK/out" 2>&1
check "all GitHub closing keywords are honoured" 1 $? "$WORK/out" "#42 carries"

setup; issue 42 "security:rejected"; pr_body "Closes #42 and Closes #42"
PR_NUMBER=1 bash scripts/security-gate.sh > "$WORK/out" 2>&1
...
# ---------------------------------------------------------------------------
echo
echo "scripts/security-verdict.sh — close-guard"
# ---------------------------------------------------------------------------

setup; issue 42 "type:story" "security:rejected"
MODE=close-guard ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
...
setup; issue 42 "type:story" "security:approved"
MODE=close-guard ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
...
# ---------------------------------------------------------------------------
echo
echo "scripts/security-verdict.sh — rejection ladder"
# ---------------------------------------------------------------------------

setup; issue 42 "security:rejected"; rejections 1
MODE=ladder ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
check "one rejection does not escalate" 0 $? "$WORK/out" "no escalation"

setup; issue 42 "security:rejected"; rejections 2
MODE=ladder ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
check "two rejections do not escalate" 0 $? "$WORK/out" "no escalation"

setup; issue 42 "security:rejected"; rejections 3
MODE=ladder ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
...
setup; issue 42 "security:rejected" "needs-human"; rejections 4
MODE=ladder ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
check "does not re-escalate an item already flagged" 0 $? "$WORK/out" "Already escalated"

setup; issue 42 "security:rejected"; rejections 3
MODE=ladder ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
assert "threshold read from role-packs/security/policy.yaml" grep "threshold 3" "$WORK/out"

setup; issue 42 "security:rejected"; rejections 3
printf 'verdict:\n  rejection_escalation_threshold: 5\n' > "$WORK/policy.yaml"
SECURITY_POLICY="$WORK/policy.yaml" MODE=ladder ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
check "policy file overrides the default threshold" 0 $? "$WORK/out" "threshold 5"

setup
MODE=nonsense ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
check "an unknown mode is a hard error" 2 $? "$WORK/out" "unknown MODE"

# ---------------------------------------------------------------------------
echo
echo "scripts/security-verdict.sh — return-to-ready"
# ---------------------------------------------------------------------------
...
setup
LABELS_JSON="$(labels_json status:in-review security:rejected)" \
  MODE=return-to-ready ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
...
setup
LABELS_JSON="$(labels_json status:ready security:rejected)" \
  MODE=return-to-ready ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
...
setup
LABELS_JSON="$(labels_json status:blocked security:rejected)" \
  MODE=return-to-ready ISSUE=42 bash scripts/security-verdict.sh > "$WORK/out" 2>&1
